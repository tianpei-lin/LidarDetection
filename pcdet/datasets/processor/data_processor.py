...
class DataProcessor(object):
    ...
    def transform_points_to_dynamic_voxels(self, data_dict=None, config=None, voxel_generator=None):
        ...
        bev_coordinate = torch.zeros(
            (self.max_points_num, 3), dtype=torch.float32)
        bev_local_coordinate = torch.zeros(
            (self.max_points_num, 3), dtype=torch.float32)
        ...
        bev_mapping_vf = torch.zeros(
            (self.max_num_of_voxels, 3), dtype=torch.int32)
    ...
    def sample_points(self, data_dict=None, config=None):
        ...
        if num_points < len(points):
            ...
            choice = np.arange(0, len(points), dtype=np.int32)
            if len(choice) > num_points - len(points):
